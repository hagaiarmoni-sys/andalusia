...
def build_word_doc(itinerary, hop_kms, maps_link, ordered_cities, days, prefs, parsed_requests, is_car_mode=False, result=None):
    ...
    # Trip start date (if available from the Streamlit app)
    start_date = None
    try:
        ...
        if start_date is None:
            print("⚠️ WARNING: start_date is None! Check session state.")
    except Exception as e:
        start_date = None
        print(f"❌ ERROR getting start_date: {e}")
    ...
    if is_star_hub and itinerary:
        ...
        if start_date_obj and end_date_obj:
            try:
                ...
                checkout_str = end_date_obj.strftime('%d %b %Y') if hasattr(end_date_obj, 'strftime') else str(end_date_obj)
    ...
    for idx, day in enumerate(itinerary):
        ...
        if start_date and day_num:
            ...
            day_label = f'📆  DAY {day_num}: {date_str} – {city}'
        else:
            day_label = f'📆  DAY {day_num}: {city}'
        ...
        # ═══════════════════════════════════════════════════════════════
        # HOTELS - Colorful recommendation boxes (ONLY ON FIRST DAY IN CITY)
        # ═══════════════════════════════════════════════════════════════
        ...
        is_first_day_in_city = (day_in_city == 1)
        
        if hotels and is_first_day_in_city and any(h.get('name') and 'Hotels in' not in h.get('name', '') for h in hotels):
            hotel_header = doc.add_heading('', 3)
            
            # ✅ NEW: Add nights count to header
            nights_text = f" ({total_days_in_city} night{'s' if total_days_in_city > 1 else ''})" if total_days_in_city > 1 else ""
            ...
            if is_return_to_start:
                ...
                hotel_run = hotel_header.add_run(f'🏨  WHERE TO STAY IN {overnight.upper()}{nights_text}')
            ...
            for hotel in hotels[:3]:
                ...
                checkin = None
                checkout = None
                nights = total_days_in_city  # ✅ FIX: Use actual nights count
                ...
                if day_date_obj:
                    ...
                    checkout = checkin + timedelta(days=nights)  # ✅ FIX: Use nights
                elif day_date_str:
        ...
        # ═══════════════════════════════════════════════════════════════
        # ROUTE STOPS - Hidden gems along the way (on travel days)
        # ═══════════════════════════════════════════════════════════════
        
        route_stops = day.get("route_stops", [])
        if route_stops:
            stops_header = doc.add_heading('', 3)
            stops_run = stops_header.add_run('🛣️  STOPS ALONG THE WAY')
            stops_run.font.size = Pt(14)
            stops_run.font.color.rgb = RGBColor(155, 89, 182)  # Purple
            
            stops_intro = doc.add_paragraph()
            stops_intro.paragraph_format.left_indent = Inches(0.3)
            intro_run = stops_intro.add_run('Recommended stops when driving to your next destination:')
            intro_run.font.size = Pt(10)
            intro_run.italic = True
            intro_run.font.color.rgb = RGBColor(127, 140, 141)
            ...
            for stop in route_stops:
                ...
                stop_para.paragraph_format.left_indent = Inches(0.3)
                ...
                stop_icon = stop_para.add_run(f'{icon}  ')
                stop_icon.font.size = Pt(12)
                
                stop_name = stop_para.add_run(stop.get('name', '?'))
                stop_name.bold = True
                stop_name.font.size = Pt(11)
                stop_name.font.color.rgb = RGBColor(44, 62, 80)
                
                # Type badge
                type_text = stop_para.add_run(f"  ({stop_type.replace('_', ' ').title()})")
                type_text.font.size = Pt(9)
                type_text.font.color.rgb = RGBColor(127, 140, 141)
                ...
                if highlight:
                    ...
                    highlight_run = highlight_para.add_run(f'✨ {highlight}')
                    highlight_run.font.size = Pt(10)
                    highlight_run.italic = True
                    highlight_run.font.color.rgb = RGBColor(243, 156, 18)
                ...
                info_run = info_para.add_run(f'⏱️ ~{time_min} min visit  •  🚗 +{detour}km detour')
