...
def load_json(filepath):
    ...
    try:
        # Check if file exists in current dir or data subdir
        target_path = filepath
        if not os.path.exists(target_path):
             target_path = os.path.join("data", filepath)
...
def main():
    ...
    attractions_data = load_json("andalusia_attractions_filtered.json")
    hotels_data = load_json("andalusia_hotels_osm.json")
    restaurants_data = load_json("restaurants_andalusia.json")
    
    st.markdown("---")
    
    # Validate critical data
    if not attractions_data:
        st.error("❌ CRITICAL: No attractions data loaded! Please check your 'data' folder.")
        st.stop()
    
    # Route to appropriate page
    if page == "Plan a Trip":
        from trip_planner_page import trip_planner_page
        trip_planner_page(default_prefs)
