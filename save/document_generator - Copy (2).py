...
print("="*80)
print("🔥🔥🔥 PHOTO-ENABLED DOCUMENT_GENERATOR LOADED! 🔥🔥🔥")
print("="*80)
...
def build_word_doc(itinerary, hop_kms, maps_link, ordered_cities, days, prefs, parsed_requests, is_car_mode=False, result=None):
    ...
    try:
        ...
        print(f"🔍 DEBUG document_generator: start_date = {start_date}")
            ...
            if attractions:
                ...
                for idx, attr in enumerate(attractions, 1):
                    ...
                    if attr.get('local_photo_path'):
                        ...
                        print(f"DEBUG: Using local_photo_path: {relative_path}")
                        
                        # Convert relative path to absolute
                        if not os.path.isabs(relative_path):
                            # It's relative (like "photos\ChIJ...jpg")
                            # Convert to: C:\...\andalusia-app\data\photos\ChIJ...jpg
                            photo_path = os.path.join(
                                r'C:\Users\hagai\PycharmProjects\pythonProject4\andalusia-app\data',
                                relative_path
                            )
                        else:
                            # Already absolute
                        ...
                        print(f"DEBUG: Converted to: {photo_path}")
                        print(f"DEBUG: File exists: {os.path.exists(photo_path)}")
                    ...
                    # Option 2: Construct path from photo_references (SMART!)
                        ...
                        if place_id:
                            ...
                            photo_path = os.path.join(
                                r'C:\Users\hagai\PycharmProjects\pythonProject4\andalusia-app\data\photos',
                                photo_filename
                            )
                            print(f"DEBUG: POI '{attr_name}' - place_id: {place_id}")
                    ...
                    if photo_path and os.path.exists(photo_path):
                        try:
                            print(f"DEBUG: Adding photo to document!")
                            ...
                            print(f"DEBUG: Photo added successfully!")
                            
                        except Exception as e:
                            # If photo fails, just continue without it
                            print(f"DEBUG: ERROR adding photo: {e}")
                            pass
                    elif photo_path:
