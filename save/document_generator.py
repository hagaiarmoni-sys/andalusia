    ...
    for idx, day in enumerate(itinerary):
        ...
        if lunch or dinner:
            ...
            if lunch:
                ...
                # ✅ FIX: Always show rating with review count
                if lunch.get('rating'):
                    rating_text = f"  •  ⭐ {lunch['rating']}"
                    
                    # Try multiple sources for review count
                    reviews_count = lunch.get('reviews_count', 0) or lunch.get('user_ratings_total', 0)
                    
                    # ✅ NEW: Parse from topic field: "565 Lebanese, Moroccan"
                    if reviews_count == 0:
            ...
            if dinner:
                ...
                if dinner.get('rating'):
                    ...
                    # Try multiple sources for review count
                    reviews_count = dinner.get('reviews_count', 0) or dinner.get('user_ratings_total', 0)
