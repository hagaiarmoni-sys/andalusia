...
def display_itinerary(result, prefs, days, attractions, hotels, restaurants):
    ...
    if is_star_hub and base_hotels:
        ...
        for hotel in base_hotels:
            booking_link = f"[Booking](https://www.booking.com/search.html?ss={hotel.get('name', '')})"
            airbnb_link = f"[Airbnb](https://www.airbnb.com/s/{hotel.get('name', '')}/homes)"
    ...
    for day_data in itinerary:
        ...
        if day_type == "driving_day":
            ...
            if city != overnight:
                ...
                st.markdown(f"## Day {day_num} 📍 {city} (returning to {overnight})")
            else:
